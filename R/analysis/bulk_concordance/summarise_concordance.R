...
input_dir = file.path(base_dir, "analysis/run_concordance")
...
dat = dats %>%
  ...
  mutate_at(vars(sc_test, shuffle_replicates, bulk_test),
            ~ gsub("^.*=|\\.rds", "", .))

# save results
output_file = "data/analysis/bulk_concordance/concordance_summary.rds"
