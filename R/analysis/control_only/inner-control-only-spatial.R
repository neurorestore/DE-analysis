...
for (comparison_name in names(comparisons)) {
  ...
  # filter to label of interest
  meta0 %<>%
    mutate(idx = row_number()) %>%
    filter(grepl(args$label, label))
  expr0 = expr[, meta0$idx]
  
  # skip if there aren't enough replicates
  if (n_distinct(meta0$replicate) < 6) {
    message("skipping comparison: ", comparison , " (not enough replicates) ...")  
    next
  }
  
  # set barcode as column
  meta0 %<>%
    as.data.frame() %>%
    mutate(new_barcode = colnames(expr0))
  ...
  # add to results
  results[[length(results) + 1]] = mutate(DE, comparison = comparison_name)
}
